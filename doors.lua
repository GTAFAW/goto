if not ExecutorSupport then print("[YEHAX] Loading stopped, please use the official loadstring for YEHAX. (ERROR: ExecutorSupport == nil)") return end
...
function Script.Functions.UpdateRPC()
    ...
    BloxstrapRPC.SetRichPresence({
        details = "Playing DOORS [ YEHEAX v3 ]",
...
do
    ...
    function Script.Functions.DoorESP(room)
        ...
        if door then
            ...
            local doorState = if opened then "[开]" elseif locked then "[锁]" else ""
            ...
            Script.FeatureConnections.Door[doorIdx] = door:GetAttributeChangedSignal("Opened"):Connect(function()
                if doorEsp then doorEsp.SetText(string.format("门 %s [开]", doorNumber)) end
                if Script.FeatureConnections.Door[doorIdx] then Script.FeatureConnections.Door[doorIdx]:Disconnect() end
            end)
        end
    end 
    ...
    function Script.Functions.ChestESP(chest)
        local text = chest.Name:gsub("宝箱", ""):gsub("_Vine", ""):gsub("_Small", "")
        local locked = chest:GetAttribute("Locked")
        local state = if locked then "[锁]" else ""
        ...
        local waitForSafeExit; waitForSafeExit = function()
            if child.Name == "A120" then
                ...
                repeat task.wait(.15)
                    ...
                    if didPlayerSeeEntity == true and isSafe == true then
                        task.spawn(function() 
                            Script.Functions.Log({
                                ...
                                LinoriaMessage = "[" .. NotifPrefix .. "]离开柜子实体已经走远了."
                            }, Toggles.AutoWardrobeNotif.Value)
                        end)
                        ...
                        if isSafe == true and not child:IsDescendantOf(workspace) then 
                            task.spawn(function() 
                                Script.Functions.Log({
                                    ...
                                    LinoriaMessage = "[" .. NotifPrefix .. "]离开柜子实体已被删除."
                                }, Toggles.AutoWardrobeNotif.Value)
                            end)
                    ...
                    if not alive then  
                        if Toggles.AutoWardrobeNotif.Value then Script.Functions.Log("[" .. NotifPrefix .. "] 停止(开都开不明白重开吧).") end             
                        task.spawn(function() 
                            Script.Functions.Log({
                                Title = NotifPrefix,
                                Description = "Stopping (you died)",

                                LinoriaMessage = "[" .. NotifPrefix .. "] 停止(开都开不明白,笑死我了)."
                            }, Toggles.AutoWardrobeNotif.Value)
                        end)
            ...
            repeat task.wait()
                task.spawn(function() 
                    Script.Functions.Log({
                        ...
                        LinoriaMessage = "[" .. NotifPrefix .. "] 等Ambush来回...",
                    }, Toggles.AutoWardrobeNotif.Value)
                end)
                ...
                if success then
                    task.spawn(function() 
                        Script.Functions.Log({
                            ...
                            LinoriaMessage = "[" .. NotifPrefix .. "] 等待Ambush离开...",
                        }, Toggles.AutoWardrobeNotif.Value)
                    end)
            ...
            Script.FeatureConnections.Character["ChildAdded"] = character.ChildAdded:Connect(function(child)
                if child:IsA("Tool") and child.Name:match("LibraryHintPaper") then
                    ...
                    if Toggles.NotifyPadlock.Value and count < 5 then
                        Script.Functions.Alert({
                            Title = "Library Code",
                            Description = string.format("密码: %s", output),
                        })

                        if Toggles.NotifyChat.Value and count == 0 then
                            RBXGeneral:SendAsync(string.format("密码: %s", output))
                        end
                    end
                end
            end)
...
local MiscGroupBox = Tabs.Main:AddRightGroupbox("不知道") do
...
local AntiEntityGroupBox = Tabs.Exploits:AddLeftGroupbox("防实体") do
    ...
    AntiEntityGroupBox:AddToggle("AntiDupe", {
        Text = "防" .. (isBackdoor and "虚空" or "假门"),
        Default = false
    })

    AntiEntityGroupBox:AddToggle("AntiEyes", {
        Text = "防" .. (isBackdoor and "影子人" or "眼睛"),
        Default = false
    })
...
local ESPTabBox = Tabs.Visuals:AddLeftTabbox() do
    local ESPTab = ESPTabBox:AddTab("显示") do
...
task.spawn(function()
    ...
    elseif isFools then
        ...
        local Fools_AntiEntityGroupBox = Tabs.Floor:AddRightGroupbox("防实体") do
...
MenuGroup:AddToggle("ExecuteOnTeleport", { Default = false, Text = "传送自动执行", Visible = ExecutorSupport["queue_on_teleport"] })
MenuGroup:AddToggle("KeybindMenuOpen", { Default = false, Text = "键位菜单", Callback = function(value) Library.KeybindFrame.Visible = value end})
MenuGroup:AddToggle("ShowCustomCursor", {Text = "修复指针", Default = true, Callback = function(Value) Library.ShowCustomCursor = Value end})
...
CreditsGroup:AddLabel("Yex - 脚本修复/汉化")
CreditsGroup:AddLabel("小天 - 白名单")
...
SaveManager:SetFolder("YEHAX/doors")
